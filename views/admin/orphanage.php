  ...
  <div class="overflow-x-auto">
    <?php if($result->status): ?>
                <table class="w-full whitespace-no-wrap">
                  ...
                  >
                  <?php foreach($result->records as $orphanage ): ?>
                        ...
                        <div class="flex items-center text-sm">
                          ...
                          <div>
                            <p class="text-xs text-gray-600 dark:text-gray-400">
                              <?php echo $orphanage->name?>
                            </p>
                          </div>
                        </div>
                      </td>
                      <td class="px-4 py-3 text-sm">
                        <?php echo $orphanage->adminId?>
                      </td>
                      <td class="px-4 py-3 text-xs">
                        <!-- <span
                          class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100"
                        > -->
                        <?php echo $orphanage->district?>
                        <!-- </span> -->
                      </td>
                      <td class="px-4 py-3 text-sm">
                        <?php echo $orphanage->area?>
                      </td>
                      <td class="px-4 py-3">
...
<div id="add-orphanage-form" style="position: fixed; z-index: 10; top: 10%; left: 35%;
            background-color: #4b88a2; display: none; width: 40%;" 
    class="p-10" >
          <?php include('../views/admin/forms/addorphanage.php') ?>
</div>
...
<script>
  ...
  var menu = {
    ...
    showAdd: () => {
      ...
      document.getElementById('add-orphanage-form').style.display = 'block';
    },
    close: () => {
      document.getElementById('overlay').style.display = 'none';
      document.getElementById('add-orphanage-form').style.display = 'none';
    }
  }
