var cal = {
  ...
  init : () => {
    ...
    document.getElementById('overlay').onclick = cal.close;
  ...
  show : (el) => {
    ...
    console.log(`${cal.sYear}-${cal.sMth > 9 ? '': '0'}${cal.sMth + 1}-${cal.sDay > 9 ? '': '0'}${cal.sDay}`);
    cal.hfDate.value = `${cal.sYear}-${cal.sMth > 9 ? '': '0'}${cal.sMth + 1}-${cal.sDay > 9 ? '': '0'}${cal.sDay}`;

    console.log(cal.hfDate.value);
